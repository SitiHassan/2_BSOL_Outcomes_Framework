---
title: "Outcomes Framework Process Documentation"
date: "`r Sys.Date()`"
author: Siti Hassan
...
## Build Local Authority registered-level data {#ch1-sec-build-la-reg-level-data}
...
## Create additional datasets {#ch1-sec-create-additional-datasets}
...
2.  **Does my data have ethnicity but not IMD?** If yes then you need **2** splits,

-   Ethnicity only (IMD = '999')
-   Neither in the grouping (IMD and Ethnicity = '999')

3.  **Does my data have IMD but not ethnicity?** If yes then you need **2** splits,

-   IMD Only (Ethnicity = '999')
-   Neither in the grouping (IMD and Ethnicity = '999')

4.  **Does my data have neither IMD or Ethnicity?** If yes then you have **1** split,
...
### ICB geography {#ch2-sec-icb-geography}
...
We would like to create additional datasets in the following combinations to enable calculation of values by IMD only, ethnicity only, IMD and ethnicity (already built in [Step 4.7](#ch2-sec-insert-data-into-staging-table)), and with no IMD and ethnicitty splits to get overall values.
...
## Create dataset with both IMD and Ethnicity collapsed to 'All' {#ch2-sec-create-dataset-with-imd-ethnicity-collapsed-to-all}
...
Union all datasets (i.e., from tables created in [Step 4.7](#ch2-sec-insert-data-into-staging-table), [Step 4.8](#ch2-sec-create-dataset-with-imd-collapsed-to-all), [Step 4.9](#ch2-sec-create-dataset-with-ethnicity-collapsed-to-all), [Step 4.10](#ch2-sec-create-dataset-with-imd-ethnicity-collapsed-to-all) and insert them into the destination table (e.g., `[EAT_Reporting_BSOL].[OF].[OF2_Indicator_SQL_Data])`.
...
## Build Local Authority registered-level data {#ch3-sec-build-la-reg-level-data}

Use the locality registered -level data built in the previous step, update the `aggregation_id` column with the local authority registered code by assigning each locality accordingly, and finally aggregate the data by grouping on the required columns.

Then, Insert this dataset into the staging table created in [Step 5.1](#ch3-sec-creating-staging-table).
...
-- Take locality registered level data from the staging NDA data and update the local authority 
DROP TABLE IF EXISTS #la_data
SELECT * INTO #la_data
...
## Create dataset with IMD collapsed to 'All' {#ch3-sec-create-dataset-with-imd-collapsed-to-all}
...
## Create dataset with Ethnicity collapsed to 'All' {#ch3-sec-create-dataset-with-ethnicity-collapsed-to-all}
...
## Create dataset with both IMD and Ethnicity collapsed to 'All' {#ch3-sec-create-dataset-with-imd-ethnicity-collapsed-to-all}
...
Union all datasets (i.e., from tables created in [Step 5.12](#ch3-sec-create-dataset-with-imd-collapsed-to-all), [Step 5.13](#ch3-sec-create-dataset-with-ethnicity-collapsed-to-all), [Step 4.9](#ch2-sec-create-dataset-with-ethnicity-collapsed-to-all), [Step 5.14](#ch3-sec-create-dataset-with-imd-ethnicity-collapsed-to-all) and insert them into the destination table (e.g., `[EAT_Reporting_BSOL].[OF].[OF2_Indicator_SQL_Data])`.
